...
func init() {
	...
	rootCmd.SetVersionTemplate("File Browser Next version {{printf \"%s\" .Version}}\n")
...
var rootCmd = &cobra.Command{
	Use:   "filebrowser",
	Short: "A modern, stylish web-based file browser",
	Long: `File Browser Next CLI lets you create the database to use with File Browser Next,
manage your users and all the configurations without accessing the
web interface.

If you've never run File Browser Next, you'll need to have a database for
...
If "--config" is not specified, File Browser Next will look for a configuration
...
Also, if the database path doesn't exist, File Browser Next will enter into
...
user created with the credentials from options "username" and "password".`,
	RunE: withViperAndStore(func(_ *cobra.Command, _ []string, v *viper.Viper, st *store) error {
		...
		log.Println("Starting File Browser Next...")
...
func getServerSettings(v *viper.Viper, st *storage.Storage) (*settings.Server, error) {
	...
	if set, err := st.Settings.Get(); err == nil && set.Signup {
		...
		if !set.CreateUserDir && scopeIsRoot {
			...
			log.Println("WARNING: delete all files File Browser Next serves, including other users' files.")
